defmodule Aoc.Year2022.Day06.TuningTrouble do
  @moduledoc """
  ...
  *device*. He says that it has many fancy features, but the most important one to
  ...
  `mjqjpqmgbljsphdztnvjfqwrcgsmlb`After the first three characters (`mjq`) have been received, there haven't been
  ...
  *How many characters need to be processed before the first start-of-packet
  ...
  def part_1(input), do: input |> String.graphemes() |> start_of_packet(4)

  def part_2(input), do: input |> String.graphemes() |> start_of_packet(14)

  def start_of_packet(buffer, check_length, index \\ 0)

  def start_of_packet(buffer, check_length, index) when is_binary(buffer),
    do: start_of_packet(String.graphemes(buffer), check_length, index)

  def start_of_packet(buffer, check_length, index) when is_list(buffer) do
    cond do
      buffer |> Enum.take(check_length) |> Enum.uniq() |> length() == check_length ->
        index + check_length

      true ->
        start_of_packet(tl(buffer), check_length, index + 1)
    end
  end
end
