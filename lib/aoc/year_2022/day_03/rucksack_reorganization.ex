defmodule Aoc.Year2022.Day03.RucksackReorganization do
  @moduledoc """
  ...
  *priority*:
  ...
  def part_2(input) do
    ...
    |> read_input()
    |> Enum.chunk_every(3)
    |> Enum.map(&parse_groups(&1))
    |> Enum.map(& &1.priority)
    |> Enum.sum()
  end

  defp read_input(input), do: input |> String.trim() |> String.split("\n")

  @priorties (Enum.map(?a..?z, fn char -> {<<char::utf8>>, char - 96} end) ++
                Enum.map(?A..?Z, fn char -> {<<char::utf8>>, char - 38} end))
             |> Map.new()

  def parse_rucksack(rucksack) do
    length = div(String.length(rucksack), 2)
    {a, b} = String.split_at(rucksack, length)

    a = String.graphemes(a)
    b = String.graphemes(b)

    [common] = MapSet.intersection(MapSet.new(a), MapSet.new(b)) |> Enum.to_list()

    priority = Map.get(@priorties, common)

    %{common: common, priority: priority}
  end

  def parse_groups([a, b, c]) do
    a = String.graphemes(a)
    b = String.graphemes(b)
