defmodule Aoc.Year2022.Day07.NoSpaceLeftOnDevice do
  ...
  *total size* of each directory. The total size of a directory is the sum of the
  ...
  def part_2(input) do
    ...
    [{"/", total_size} | dirs] = dir_sizes({"/", fs}) |> Enum.reverse()

    size_needed = total_size - 40_000_000

    dirs
    |> Enum.filter(fn {_name, size} -> size >= size_needed end)
    |> Enum.min_by(fn {_name, size} -> size - size_needed end)
    |> elem(1)
  end

  def dir_sizes(fs, acc \\ [])

  def dir_sizes({_file, size}, acc) when is_integer(size), do: acc

  def dir_sizes({cwd, fs}, acc) do
    current = {cwd, dir_size(fs)}

    Enum.reduce(fs, [current | acc], fn child, acc ->
      dir_sizes(child, acc)
    end)
  end

  def dir_size(size) when is_integer(size), do: size

  def dir_size(dir) do
    Enum.map(dir, fn
      {_dir, %{} = children} -> dir_size(children)
      {_file, size} -> size
    end)
    |> Enum.sum()
  end
  ...
  defp parse_line("$ ls", {fs, cwd}), do: {fs, cwd}
  defp parse_line("$ cd ..", {fs, cwd}), do: {fs, Enum.drop(cwd, -1)}
  defp parse_line("$ cd " <> dir, {fs, cwd}), do: {fs, cwd ++ [dir]}
  defp parse_line("dir " <> dir, {fs, cwd}), do: {update_in(fs, cwd, &Map.put(&1, dir, %{})), cwd}

  defp parse_line(file, {fs, cwd}) do
    [size, filename] = String.split(file, " ")
    size = String.to_integer(size)
    {update_in(fs, cwd, &Map.put(&1, filename, size)), cwd}
  end

  defp parse_line(_cmd, acc), do: acc
end
